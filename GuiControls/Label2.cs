...
namespace GuiControls
{
    ...
    public class Label2 : ControlBase
    {
        ...
        public Label2(string name, Vector2 position, ContentAlignment alignment, Vector2 size, string text, string fontName, Color textColor, Color backColor, float layerDepth = 0.0f, IControl parent = null) :
            base(name, position, alignment, size, "", "", null, layerDepth, parent)
        {
            //_parent = parent;
            //Name = name;
            _fontName = fontName;
        ...
        public override void Draw(Matrix? transform = null)
        {
            ...
            if (_backColor != null)
            {
                spriteBatch.FillRectangle(ActualDestinationRectangle, _backColor, LayerDepth);
            }

            //spriteBatch.DrawString(_font, _text, TopLeft.ToVector2(), _textColor, 0.0f, _origin, _scale, SpriteEffects.None, LayerDepth);
            spriteBatch.DrawString(_font, _text, TopLeft.ToVector2(), _textColor, 0.0f, Vector2.Zero, _scale, SpriteEffects.None, LayerDepth);
